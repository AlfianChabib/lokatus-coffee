...
import { KeySquare, LoaderCircle, PanelLeft } from "lucide-react";

export default function AdminHeader() {
  const { toggleSidebar, open } = useSidebar();
  const { data: passkey, isLoading } = usePasskey();
  ...
  return (
    <nav className="flex h-14 items-center justify-between border-b bg-white px-2 backdrop-blur-sm md:px-4">
      ...
      <div>
        <Button variant={"outline"} size={"sm"} className="items-center space-x-1 border py-0">
          {isLoading ? (
            <LoaderCircle className="animate-spin" />
          ) : (
            <p className="text-sm font-medium uppercase">{passkey}</p>
          )}
          <KeySquare />
        </Button>
      </div>
